...
cols<-c('green','purple','black','grey','blue','lightblue')
plot(c(0:30),df[,1],type="l",lty=ltypes[1],xlim=c(0,30),ylim=c(0,1),
     ylab="Power",xlab=expression(sigma),
     lwd=1,cex.lab=1.3,cex.axis=1.1,col=cols[1],
     main="Power for the cubic dependence")
points(c(0:30),df[,1],pch=ptypes[1],col=cols[1])
for(i in 2:6){
  par(new=TRUE)
  plot(c(0:30),df[,i],type="l",xlim=c(0,30),ylim=c(0,1),lty=ltypes[i],
       axes=FALSE,xlab="",ylab="",col=cols[i])
  points(c(0:30),df[,i],pch=ptypes[i],col=cols[i])
} 
legend("topright",legend=c('|r|','|r_s|','rho_max',
                           'rho_dist','r1','MIC'),
       pch=ptypes,lty=ltypes,
       lwd=c(1,1,1,1,1),col=cols,cex=1.3)
